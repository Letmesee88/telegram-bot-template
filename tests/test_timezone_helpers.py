...
async def test_get_user_tzinfo_fallback_default(monkeypatch):
    ...
    orig = getattr(cfg_mod.settings, "DEFAULT_TZ", None)
    ...
    assert tzinfo.key in {"Europe/Moscow", "Europe/Moscow"}
    # Restore (monkeypatch auto-restores after test)


@pytest.mark.asyncio
async def test_get_user_tzinfo_invalid_fallback_utc(monkeypatch):
    users_mod = importlib.import_module("bot.services.users")
    async def _fake_get_timezone(session, uid):
        return "Not/AZone"
    monkeypatch.setattr(users_mod, "get_timezone", _fake_get_timezone, raising=False)
...
@pytest.mark.asyncio
async def test_today_local_utc_dates_kiritimati(monkeypatch):
...
@pytest.mark.asyncio
async def test_today_local_utc_dates_los_angeles(monkeypatch):
...
@pytest.mark.asyncio
async def test_today_local_utc_dates_utc_single_date(monkeypatch):
    users_mod = importlib.import_module("bot.services.users")
    async def _fake_get_tzinfo(session, uid):
        return timezone.utc
    monkeypatch.setattr(users_mod, "get_user_tzinfo", _fake_get_tzinfo, raising=False)

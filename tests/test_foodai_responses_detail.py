...
async def test_responses_payload_includes_detail_high(monkeypatch: pytest.MonkeyPatch):
    ...
    # Stub Telegram file resolving
    monkeypatch.setattr(svc, "_tg_file_url", lambda fid: "https://example.com/file.jpg", raising=False)
    # Precheck says it's food
    monkeypatch.setattr(svc, "_foodness_photo", lambda url: True, raising=False)
    ...
    async def _fake_openai_request(kind: str, payload: dict):
        nonlocal captured
        captured = payload
        # Return a minimal valid JSON for normalize step
        return json.dumps({
            "title": "Тест блюдо",
            "calories": 100,
            "protein_g": 10.0,
